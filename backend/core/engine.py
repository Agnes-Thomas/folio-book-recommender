"""
Hybrid Book Recommendation Engine — Pre-processed data version.
Loads from backend/data/ (committed to repo) — no CSV downloading at runtime.
Peak RAM: ~150MB.
"""
...
class RecommendationEngine:
    ...
    def _load_books(self):
        path = os.path.join(DATA_DIR, "books_clean.csv")
        self.books = pd.read_csv(path).reset_index(drop=True)

        # Load user activity
        act_path = os.path.join(DATA_DIR, "user_activity.json")
        with open(act_path) as f:
            raw = json.load(f)
        # JSON keys are strings — convert to int
        self.user_activity = {int(k): v for k, v in raw.items()}
        print(f"  Books: {len(self.books):,}  |  Users: {len(self.user_activity):,}")
    ...
    def _train_svd(self):
        path = os.path.join(DATA_DIR, "ratings_sample.csv")
        sample = pd.read_csv(path)
        print(f"  Training SVD on {len(sample):,} ratings…")
    ...
    def _train_meta(self):
        def feats(preds):
            ...
            for uid, iid, true_r, est, _ in preds:
                ...
                X.append([est / 5, pop, act]); y.append(true_r)
            return X, y
        Xtr, ytr = feats(self.svd_model.test(self.trainset.build_testset()))
        Xte, yte = feats(self.svd_model.test(self.testset))
        self.reg_model = LinearRegression().fit(Xtr, ytr)
        print(f"  ✅ Meta-regression R²={self.reg_model.score(Xte, yte):.4f}")
    ...
    def _score_candidates(self, candidates, user_id: int):
        uw  = min(self.user_activity.get(user_id, 0) / 50, 1.0)
        ...
        for idx, cos_sim in candidates:
            ...
            final      = round(0.7 * (hybrid + 0.1*pop) + 0.3 * (reg_pred/5), 4)
            results.append({
                ...
                "final_score":   final,
                "explanation":   f"{'CF-heavy' if uw>0.6 else 'Content-heavy'} (uw={uw:.2f})",
                "source":        "LOCAL",
            })
    ...
    def _cold_start_search(self, query: str, top_n: int = 5):
        ...
        if not data or not data.get("docs"): return None, []
        doc  = data["docs"][0]
        ...
        recs  = [{
            "book_idx": idx, "title": self.books.iloc[idx]["title"],
            "authors": self.books.iloc[idx]["authors"],
            "average_rating": float(self.books.iloc[idx]["average_rating"])
                              if pd.notna(self.books.iloc[idx]["average_rating"]) else None,
            "isbn": str(self.books.iloc[idx]["isbn"])
                    if pd.notna(self.books.iloc[idx]["isbn"]) else None,
            "cover_url": self._cover_url(self.books.iloc[idx]),
            "publication_year": int(self.books.iloc[idx]["original_publication_year"])
                                if pd.notna(self.books.iloc[idx]["original_publication_year"]) else None,
            "content_score": round(float(s), 4), "collab_score": None,
            "final_score": round(float(s), 4),
    ...
    def get_model_stats(self):
        return {"books": len(self.books), "users": len(self.user_activity),
                "tfidf_features": self.tfidf_matrix.shape[1], "svd_factors": 50}
